...
window.exportRetakesToExcel = () => {
  const rows = Array.from(document.querySelectorAll('#retakesAdminBody tr')).map(tr => {
    const cells = tr.querySelectorAll('td');
    if (!cells.length) return null;
    return [
      cells[0]?.textContent.trim(),
  ...
  const headers = ['ID', 'Colaborador', 'Treinamento', 'Status', 'Criado em', 'Consumido em', 'Observação'];
  const BOM = '\uFEFF';
  const escape = v => {
    const s = String(v);
    return s.includes(';') || s.includes('"') || s.includes('\n') ? `"${s.replace(/"/g, '""')}"` : s;
  ...
  const dataHoje = new Date().toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' }).replace(/\//g, '-');
  a.href = url;
  a.download = `retakes_${dataHoje}.csv`;
  a.click();
  URL.revokeObjectURL(url);
};