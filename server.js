...
app.get('/api/trainings/:id/questions', requireAuth, (req, res) => res.json(db.prepare('SELECT id, category, question_text, scenario_text, option_a, option_b, option_c, option_d FROM quiz_questions WHERE training_id = ? AND active = 1 ORDER BY id ASC').all(Number(req.params.id))));
